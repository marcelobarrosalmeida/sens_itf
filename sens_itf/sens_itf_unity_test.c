...
static osens_cmd_req_t cmd_mote;
static osens_cmd_res_t ans_mote;
static osens_cmd_req_t cmd_sensor;
static osens_cmd_res_t ans_sensor;
...
void setUp(void)
{
    memset(&cmd_mote, 0, sizeof(osens_cmd_req_t));
    memset(&ans_mote, 0, sizeof(osens_cmd_res_t));
    memset(&cmd_sensor, 0, sizeof(osens_cmd_req_t));
    memset(&ans_sensor, 0, sizeof(osens_cmd_res_t));
    memset(frame, 0, OSENS_MAX_FRAME_SIZE);
...
static void test_decode_ans(uint8_t expect_size, uint8_t res_size, osens_cmd_res_t *expect_ans, osens_cmd_res_t *ans)
{
...
static void test_decode_req(uint8_t expect_size, uint8_t res_size, osens_cmd_req_t *expect_req, osens_cmd_req_t *req)
{
    TEST_ASSERT_EQUAL_UINT8(expect_size, res_size);
...
static void validate_point_value(osens_point_t *req, osens_point_t *ans)
{
    switch (req->type)
    {
    case OSENS_DT_U8:
...
static void test_OSENS_REGMAP_ITF_VERSION(void)
{
    uint8_t frame_mote[OSENS_MAX_FRAME_SIZE]   = {2, OSENS_REGMAP_ITF_VERSION, 0x00, 0x00};
    uint8_t frame_sensor[OSENS_MAX_FRAME_SIZE] = {4, OSENS_REGMAP_ITF_VERSION, OSENS_ANS_OK, OSENS_LATEST_VERSION, 0x00, 0x00};
    uint16_t crc;
    ...
    cmd_req_size = 4;
    cmd_res_size = 6;
    cmd_number = OSENS_REGMAP_ITF_VERSION;

    // enconde command req
    crc = crc16_calc(frame_mote, 2);
    buf_io_put16_tl(crc, &frame_mote[2]);
	cmd_mote.hdr.addr = OSENS_REGMAP_ITF_VERSION;
    size_mote = osens_pack_cmd_req(&cmd_mote, frame);
    TEST_ASSERT_EQUAL_UINT8(cmd_req_size, size_mote);
    TEST_ASSERT_EQUAL_INT8_ARRAY(frame_mote, frame, size_mote);
    
    // decode command req
    size_sensor = osens_unpack_cmd_req(&cmd_sensor, frame, size_mote);
    ...
    ans_sensor.hdr.addr = OSENS_REGMAP_ITF_VERSION;
    ans_sensor.hdr.status = OSENS_ANS_OK;
    ans_sensor.payload.itf_version_cmd.version = OSENS_LATEST_VERSION;
    size_sensor = osens_pack_cmd_res(&ans_sensor, frame);
    TEST_ASSERT_EQUAL_UINT8(cmd_res_size, size_sensor);
    TEST_ASSERT_EQUAL_INT8_ARRAY(frame_sensor, frame, size_sensor);

    // decode command res
    size_mote = osens_unpack_cmd_res(&ans_mote, frame, size_sensor);
...
static void test_OSENS_REGMAP_BRD_ID(void)
{
    setUp();

    cmd_req_size = 4;
    cmd_res_size = 28;
    cmd_number = OSENS_REGMAP_BRD_ID;
    ...
    ans_sensor.payload.brd_id_cmd.cabalities = OSENS_CAPABILITIES_DISPLAY |
    ...
    size_mote = osens_unpack_cmd_res(&ans_mote, frame, size_sensor);
    test_decode_ans(cmd_res_size, size_mote,&ans_sensor,&ans_mote);
    TEST_ASSERT_EQUAL_INT8_ARRAY(ans_sensor.payload.brd_id_cmd.model, ans_mote.payload.brd_id_cmd.model,OSENS_MODEL_NAME_SIZE);
    TEST_ASSERT_EQUAL_INT8_ARRAY(ans_sensor.payload.brd_id_cmd.manufactor, ans_mote.payload.brd_id_cmd.manufactor,OSENS_MANUF_NAME_SIZE);
...
static void test_OSENS_REGMAP_BRD_STATUS(void)
{
    setUp();

    cmd_req_size = 4;
    cmd_res_size = 6;
    cmd_number = OSENS_REGMAP_BRD_STATUS;
...
static void test_OSENS_REGMAP_BRD_CMD(void)
{
    setUp();

    cmd_req_size = 5;
    cmd_res_size = 6;
    cmd_number = OSENS_REGMAP_BRD_CMD;
...
static void test_OSENS_REGMAP_READ_BAT_STATUS(void)
{
    setUp();

    cmd_req_size = 4;
    cmd_res_size = 6;
    cmd_number = OSENS_REGMAP_READ_BAT_STATUS;
...
static void test_OSENS_REGMAP_WRITE_BAT_STATUS(void)
{
    cmd_req_size = 5;
    cmd_res_size = 5;
    cmd_number = OSENS_REGMAP_WRITE_BAT_STATUS;
...
static void test_OSENS_REGMAP_READ_BAT_CHARGE(void)
{
    cmd_req_size = 4;
    cmd_res_size = 6;
    cmd_number = OSENS_REGMAP_READ_BAT_CHARGE;
...
static void test_OSENS_REGMAP_WRITE_BAT_CHARGE(void)
{
    cmd_req_size = 5;
    cmd_res_size = 5;
    cmd_number = OSENS_REGMAP_WRITE_BAT_CHARGE;
...
static void test_OSENS_REGMAP_WPAN_STATUS(void)
{
    cmd_req_size = 5;
    cmd_res_size = 5;
    cmd_number = OSENS_REGMAP_WPAN_STATUS;
...
static void test_OSENS_REGMAP_WPAN_STRENGTH(void)
{
    cmd_req_size = 5;
    cmd_res_size = 5;
    cmd_number = OSENS_REGMAP_WPAN_STRENGTH;
...
static void test_OSENS_REGMAP_DSP_WRITE(void)
{
    cmd_req_size = 29;
    cmd_res_size = 5;
    cmd_number = OSENS_REGMAP_DSP_WRITE;
    ...
    size_sensor = osens_unpack_cmd_req(&cmd_sensor, frame, size_mote);
    test_decode_req(cmd_req_size, size_sensor,&cmd_mote, &cmd_sensor);
    TEST_ASSERT_EQUAL_INT8_ARRAY(cmd_mote.payload.write_display_cmd.msg, cmd_mote.payload.write_display_cmd.msg,OSENS_DSP_MSG_MAX_SIZE);
...
static void test_OSENS_REGMAP_SVR_MAIN_ADDR(void)
{
    cmd_req_size = 4;
    cmd_res_size = 21;
    cmd_number = OSENS_REGMAP_SVR_MAIN_ADDR;
...
static void test_OSENS_REGMAP_SVR_SEC_ADDR(void)
{
    cmd_req_size = 4;
    cmd_res_size = 21;
    cmd_number = OSENS_REGMAP_SVR_SEC_ADDR;
...
static void test_OSENS_REGMAP_POINT_DESC_1(void)
{
    cmd_req_size = 4;
    cmd_res_size = 20;
    cmd_number = OSENS_REGMAP_POINT_DESC_1;
    ...
    ans_sensor.payload.point_desc_cmd.type = OSENS_DT_U8;
    ans_sensor.payload.point_desc_cmd.unit = 0; // TBD
    ans_sensor.payload.point_desc_cmd.access_rights = OSENS_ACCESS_READ_ONLY;
    ...
    size_mote = osens_unpack_cmd_res(&ans_mote, frame, size_sensor);
    test_decode_ans(cmd_res_size, size_mote,&ans_sensor,&ans_mote);
    TEST_ASSERT_EQUAL_INT8_ARRAY(ans_sensor.payload.point_desc_cmd.name, ans_mote.payload.point_desc_cmd.name,OSENS_POINT_NAME_SIZE);
...
static void test_OSENS_REGMAP_POINT_DESC_2(void)
{
    cmd_req_size = 4;
    cmd_res_size = 20;
    cmd_number = OSENS_REGMAP_POINT_DESC_2;
    ...
    ans_sensor.hdr.addr = cmd_number;
    ans_sensor.hdr.status = OSENS_ANS_OK;
    memcpy(ans_sensor.payload.point_desc_cmd.name, "12345678",OSENS_POINT_NAME_SIZE);
    ans_sensor.payload.point_desc_cmd.type = OSENS_DT_FLOAT;
    ans_sensor.payload.point_desc_cmd.unit = 10; // TBD
    ans_sensor.payload.point_desc_cmd.access_rights = OSENS_ACCESS_READ_WRITE;
...
void test_OSENS_REGMAP_READ_POINT_DATA_1(void)
{
    cmd_req_size = 4;
    cmd_res_size = 10;
    cmd_number = OSENS_REGMAP_READ_POINT_DATA_1;
    ...
    ans_sensor.hdr.addr = cmd_number;
    ans_sensor.hdr.status = OSENS_ANS_OK;
    ans_sensor.payload.point_value_cmd.type = OSENS_DT_FLOAT;
...
void test_OSENS_REGMAP_WRITE_POINT_DATA_5(void)
{
    cmd_req_size = 9;
    cmd_res_size = 5;
    cmd_number = OSENS_REGMAP_WRITE_POINT_DATA_5;

    // enconde command req
    cmd_mote.hdr.addr = cmd_number;
    cmd_mote.payload.point_value_cmd.type = OSENS_DT_U32;
...
void test_OSENS_REGMAP_READ_POINT_DATA_32(void)
{
    cmd_req_size = 4;
    cmd_res_size = 8;
    cmd_number = OSENS_REGMAP_READ_POINT_DATA_32;
    ...
    ans_sensor.hdr.addr = cmd_number;
    ans_sensor.hdr.status = OSENS_ANS_OK;
    ans_sensor.payload.point_value_cmd.type = OSENS_DT_S16;
...
void test_main(void)
{
    ...
    RUN_TEST(test_OSENS_REGMAP_ITF_VERSION,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_BRD_ID,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_BRD_STATUS,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_BRD_CMD,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_READ_BAT_STATUS,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_WRITE_BAT_STATUS,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_READ_BAT_CHARGE,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_WRITE_BAT_CHARGE,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_WPAN_STATUS,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_WPAN_STRENGTH,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_DSP_WRITE,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_SVR_MAIN_ADDR,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_SVR_SEC_ADDR,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_POINT_DESC_1,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_POINT_DESC_2,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_READ_POINT_DATA_1,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_WRITE_POINT_DATA_5,__LINE__);
    RUN_TEST(test_OSENS_REGMAP_READ_POINT_DATA_32,__LINE__);
